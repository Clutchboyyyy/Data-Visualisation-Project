...
@st.cache_data(show_spinner = "Creating the plot...")
def plot_1(data):
...
def plot_6(data):
    ...
    sns.barplot(x=data["domaine"], y=data["salaire_net_median_des_emplois_a_temps_plein"], estimator=lambda x: x.mean(),
                palette="hls")
    plt.xlabel("Domaine")
...
@st.cache_data(show_spinner = "Creating the plot...")
def plot_8(data):
    sns.kdeplot(data=data, x="salaire_net_median_des_emplois_a_temps_plein", hue="domaine", shade=True, alpha=.5,
                common_norm=False)
    st.pyplot(plt.gcf(), clear_figure=True)
...
def plot_9(data):
    ...
    for attribute in situation:
        new_data[attribute] = grouped_data[grouped_data["situation"] == attribute][
            "salaire_net_median_des_emplois_a_temps_plein"].values
    x = np.arange(len(dom))
    width = 0.25
    multiplier = 0
